...
class SLAMMapper:
    ...
    def __init__(self):
        self.map = np.zeros((MAP_SIZE, MAP_SIZE), dtype=np.float32)
    ...
    def update_map(self, point_cloud):
        ...
        try:
            ...
            for p in point_cloud:
                if abs(p['x']) < 5.0 and abs(p['y']) < 5.0:
            ...
            with self.map_lock:
                ...
                self.map[robot_map_pos[1], robot_map_pos[0]] = 0.5
    ...
    def get_map_image(self):
        ...
        with self.map_lock:
            ...
            if len(self.path_history) > 1:
                ...
                for x, y in path_map:
                    colored_map[y, x] = [1.0, 0.0, 0.0, 1.0]
            
            if self.planned_path:
                path = np.array(self.planned_path)
                path_map = self.world_to_map(path[:, :2])
                for x, y in path_map:
                    colored_map[y, x] = [0.0, 1.0, 0.0, 1.0]
